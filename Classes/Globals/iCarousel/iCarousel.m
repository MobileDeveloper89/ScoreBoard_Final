...
- (CGFloat)decelerationDistance
{
    ...
    return -powf(_startVelocity, 1.0f) / (1.0f * acceleration);
}
...
- (void)didScroll
{
    ...
    if ((_scrollOffset - roundf(_scrollOffset)) > 0.1 && (_scrollOffset - roundf(_scrollOffset)) < 0.5) {
        currentIndex++;
    } else if ((_scrollOffset - roundf(_scrollOffset)) < -0.1 && (_scrollOffset - roundf(_scrollOffset)) > -0.5) {
        currentIndex--;
    }
...
{
    if (globalVolleyBallViewBigger || globalDigitalViewBigger || globalTheme1Bigger || globalTheme2ViewBigger || globalTheme3ViewBigger) {
        return NO;
    }
